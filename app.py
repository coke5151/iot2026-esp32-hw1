...
st.sidebar.divider()
records_limit = st.sidebar.slider(
...
def update_mock_data(limit):
    ...
    if "mock_df" not in st.session_state or st.session_state.mock_df is None:
        ...
        df = pd.DataFrame(
            {"timestamp": timestamps, "temperature": temps, "humidity": hums}
        )
    ...
    new_row = pd.DataFrame(
        {"timestamp": [now], "temperature": [new_temp], "humidity": [new_hum]}
    )
...
def render_dashboard_fragment():
    ...
    st.markdown(
        "Hover over the lines for exact timestamps. Drag to zoom in on specific intervals."
    )
